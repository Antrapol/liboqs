...
int KS5_KAZ_DS_KeyGen(unsigned char *kaz_ds_verify_key,
  ...
  mpz_set_str(N, KS5_KAZ_DS_SP_N, 10);
  mpz_set_str(GRg, KS5_KAZ_DS_SP_GRg, 10);
  mpz_set_str(phiGRg, KS5_KAZ_DS_SP_PHIGRg, 10);
  mpz_set_str(phiphiGRg, KS5_KAZ_DS_SP_PHIPHIGRg, 10);
  mpz_set_str(phiGg, KS5_KAZ_DS_SP_PHIGg, 10);
  mpz_set_str(q, KS5_KAZ_DS_SP_q, 10);
  mpz_set_str(GRgq, KS5_KAZ_DS_SP_GRgq, 10);
  mpz_set_str(Q, KS5_KAZ_DS_SP_Q, 10);
  mpz_set_str(phiQ, KS5_KAZ_DS_SP_PHIQ, 10);
  mpz_set_str(qQ, KS5_KAZ_DS_SP_qQ, 10);

  int nphiGg = KS5_KAZ_DS_SP_nPHIGg;
  ...
  if (ALPHABYTE == NULL) {
    printf("KS5 Keygen memory allocation failed 1\n");
    goto cleanup;
  }
  ...
  if (BBYTE == NULL) {
    printf("KS5 Keygen memory allocation failed 2\n");
    goto cleanup;
  }
  ...
  if (V1BYTE == NULL) {
    printf("KS5 Keygen memory allocation failed 3\n");
    goto cleanup;
  }
  ...
  if (V2BYTE == NULL) {
    printf("KS5 Keygen memory allocation failed 4\n");
    goto cleanup;
  }
  mpz_export(V2BYTE, &V2SIZE, 1, sizeof(char), 0, 0, V2);

  memset(kaz_ds_sign_key, 0, KS5_CRYPTO_SECRETKEYBYTES * sizeof(unsigned char));
  ...
  // je = CRYPTO_SECRETKEYBYTES - KS5_KAZ_DS_BBYTES - 1;
  // for (int i = ALPHASIZE - 1; i >= 0; i--) {
  //   kaz_ds_sign_key[je] = ALPHABYTE[i];
  //   je--;
  // }

  // Fill up the buffer from left most and leave MSB zero
  // if output is less than KS1_KAZ_DS_BBYTES
  memcpy(&(kaz_ds_sign_key[KS5_CRYPTO_SECRETKEYBYTES - BSIZE]), BBYTE, BSIZE);

  // same algo as above
  memcpy(&(kaz_ds_sign_key[KS5_CRYPTO_SECRETKEYBYTES - KS5_KAZ_DS_BBYTES -
                           ALPHASIZE]),
         ALPHABYTE, ALPHASIZE);
  ...
  // je = CRYPTO_PUBLICKEYBYTES - KS5_KAZ_DS_V2BYTES - 1;
  // for (int i = V1SIZE - 1; i >= 0; i--) {
  //   kaz_ds_verify_key[je] = V1BYTE[i];
  //   je--;
  // }
  //
  memset(kaz_ds_verify_key, 0,
         KS5_CRYPTO_PUBLICKEYBYTES * sizeof(unsigned char));

  memcpy(&(kaz_ds_verify_key[KS5_CRYPTO_PUBLICKEYBYTES - V2SIZE]), V2BYTE,
         V2SIZE);

  memcpy(&(kaz_ds_verify_key[KS5_CRYPTO_PUBLICKEYBYTES - KS5_KAZ_DS_V2BYTES -
                             V1SIZE]),
         V1BYTE, V1SIZE);
...
int KS5_KAZ_DS_SIGNATURE(unsigned char *signature, unsigned long long *signlen,
  ...
  mpz_set_str(phiphiGRg, KS5_KAZ_DS_SP_PHIPHIGRg, 10);
  mpz_set_str(phiQ, KS5_KAZ_DS_SP_PHIQ, 10);
  mpz_set_str(GRgqQ, KS5_KAZ_DS_SP_GRgqQ, 10);
  mpz_set_str(phiGRgqQ, KS5_KAZ_DS_SP_PHIGRgqQ, 10);
  mpz_set_str(qQ, KS5_KAZ_DS_SP_qQ, 10);
  mpz_set_str(phiqQ, KS5_KAZ_DS_SP_PHIqQ, 10);
  ...
  ALPHABYTE =
      (unsigned char *)malloc((KS5_KAZ_DS_ALPHABYTES) * sizeof(unsigned char));
  BBYTE = (unsigned char *)malloc((KS5_KAZ_DS_BBYTES) * sizeof(unsigned char));

  for (int i = 0; i < KS5_KAZ_DS_ALPHABYTES; i++)
    ALPHABYTE[i] = 0;
  for (int i = 0; i < KS5_KAZ_DS_BBYTES; i++)
    BBYTE[i] = 0;
  ...
  for (int i = 0; i < KS5_KAZ_DS_BBYTES; i++) {
    BBYTE[i] = sk[i + KS5_KAZ_DS_ALPHABYTES];
  }

  mpz_import(ALPHA, KS5_KAZ_DS_ALPHABYTES, 1, sizeof(char), 0, 0, ALPHABYTE);
  mpz_import(b, KS5_KAZ_DS_BBYTES, 1, sizeof(char), 0, 0, BBYTE);

  // 3) Compute HASHValue(m)
  unsigned char buf[KS5_CRYPTO_BYTES] = {0};
  HashMsg(m, mlen, buf);

  mpz_import(hashValue, KS5_CRYPTO_BYTES, 1, sizeof(char), 0, 0, buf);
  mpz_nextprime(hashValue, hashValue);

  // 4) Generate random r & ephemeral BETA
  KAZ_DS_RANDOM(KS5_KAZ_DS_SP_nPHIGg - 2, KS5_KAZ_DS_SP_nPHIGg - 1, r);
  mpz_nextprime(r, r);
  ...
  for (int i = 0; i < (int)(mlen + KS5_KAZ_DS_SBYTES); i++)
    signature[i] = 0;

  int je = mlen + KS5_KAZ_DS_SBYTES - 1;
  for (int i = mlen - 1; i >= 0; i--) {
    signature[je] = m[i];
    je--;
  }

  je = KS5_KAZ_DS_SBYTES - 1;
  for (int i = SSIZE - 1; i >= 0; i--) {
    signature[je] = SBYTE[i];
    je--;
  }

  *signlen = mlen + KS5_KAZ_DS_SBYTES;
...
int KS5_KAZ_DS_VERIFICATION(unsigned char *m, unsigned long long *mlen,
  ...
  // 1) Get all system parameters
  mpz_set_str(N, KS5_KAZ_DS_SP_N, 10);
  mpz_set_str(g, KS5_KAZ_DS_SP_G, 10);
  mpz_set_str(Gg, KS5_KAZ_DS_SP_Gg, 10);
  mpz_set_str(R, KS5_KAZ_DS_SP_R, 10);
  mpz_set_str(GRg, KS5_KAZ_DS_SP_GRg, 10);
  mpz_set_str(q, KS5_KAZ_DS_SP_q, 10);
  mpz_set_str(Q, KS5_KAZ_DS_SP_Q, 10);
  mpz_set_str(phiQ, KS5_KAZ_DS_SP_PHIQ, 10);
  mpz_set_str(GRgQ, KS5_KAZ_DS_SP_GRgQ, 10);
  mpz_set_str(phiGRgQ, KS5_KAZ_DS_SP_PHIGRgQ, 10);
  mpz_set_str(GRgqQ, KS5_KAZ_DS_SP_GRgqQ, 10);
  mpz_set_str(qQ, KS5_KAZ_DS_SP_qQ, 10);
  mpz_set_str(phiqQ, KS5_KAZ_DS_SP_PHIqQ, 10);

  // int n = KS5_KAZ_DS_SP_n;
  ...
  V1BYTE =
      (unsigned char *)malloc((KS5_KAZ_DS_V1BYTES) * sizeof(unsigned char));
  V2BYTE =
      (unsigned char *)malloc((KS5_KAZ_DS_V2BYTES) * sizeof(unsigned char));

  for (int i = 0; i < KS5_KAZ_DS_V1BYTES; i++)
    V1BYTE[i] = 0;
  for (int i = 0; i < KS5_KAZ_DS_V2BYTES; i++)
    V2BYTE[i] = 0;
  ...
  for (int i = 0; i < KS5_KAZ_DS_V2BYTES; i++) {
    V2BYTE[i] = pk[i + KS5_KAZ_DS_V1BYTES];
  }

  mpz_import(V1, KS5_KAZ_DS_V1BYTES, 1, sizeof(char), 0, 0, V1BYTE);
  mpz_import(V2, KS5_KAZ_DS_V2BYTES, 1, sizeof(char), 0, 0, V2BYTE);

  // 3) Get signature=(S, m)
  int len = smlen - KS5_KAZ_DS_SBYTES;

  SBYTE = (unsigned char *)malloc(KS5_KAZ_DS_SBYTES * sizeof(unsigned char));
  MBYTE = (unsigned char *)malloc(len * sizeof(unsigned char));

  for (int i = 0; i < KS5_KAZ_DS_SBYTES; i++)
    SBYTE[i] = 0;
  for (int i = 0; i < len; i++)
    MBYTE[i] = 0;
  ...
  for (int i = 0; i < len; i++) {
    MBYTE[i] = sm[i + KS5_KAZ_DS_SBYTES];
  }

  mpz_import(S, KS5_KAZ_DS_SBYTES, 1, sizeof(char), 0, 0, SBYTE);
...
int KS5_KAZ_DS_SIGNATURE_DETACHED(unsigned char *signature,
  ...
  // 2) Get kaz_ds_sign_key=(ALPHA, b)
  ALPHABYTE =
      (unsigned char *)calloc(KS5_KAZ_DS_ALPHABYTES, sizeof(unsigned char));
  if (ALPHABYTE == NULL) {
    printf("KS5 signing memory allocation failed 1\n");
    ret = -10;
    goto cleanup;
  }

  BBYTE = (unsigned char *)calloc(KS5_KAZ_DS_BBYTES, sizeof(unsigned char));
  if (BBYTE == NULL) {
    printf("KS5 signing memory allocation failed 2\n");
    ret = -11;
    goto cleanup;
  }

  memcpy(ALPHABYTE, sk, KS5_KAZ_DS_ALPHABYTES);
  memcpy(BBYTE, sk + KS5_KAZ_DS_ALPHABYTES, KS5_KAZ_DS_BBYTES);

  mpz_import(ALPHA, KS5_KAZ_DS_ALPHABYTES, 1, sizeof(unsigned char), 0, 0,
             ALPHABYTE);
  mpz_import(b, KS5_KAZ_DS_BBYTES, 1, sizeof(unsigned char), 0, 0, BBYTE);

  // 3) Compute HASHValue(m)
  unsigned char buf[KS5_CRYPTO_BYTES] = {0};
  HashMsg(m, mlen, buf);

  mpz_import(hashValue, KS5_CRYPTO_BYTES, 1, sizeof(unsigned char), 0, 0, buf);
  mpz_nextprime(hashValue, hashValue);
  ...
  SBYTE = (unsigned char *)malloc(SSIZE * sizeof(unsigned char));
  if (SBYTE == NULL) {
    printf("KS5 signing memory allocation failed 3\n");
    ret = -12;
    goto cleanup;
  }
  ...
  memset(signature, 0, KS5_KAZ_DS_SBYTES * sizeof(unsigned char));
  memcpy(signature + (KS5_KAZ_DS_SBYTES - SSIZE), SBYTE, SSIZE);

  *signlen = KS5_KAZ_DS_SBYTES;
...
int KS5_KAZ_DS_VERIFICATION_DETACHED(const unsigned char *m, unsigned int mlen,
  ...
  // 2) Get kaz_ds_verify_key=(V1, V2)
  V1BYTE = (unsigned char *)calloc(KS5_KAZ_DS_V1BYTES, sizeof(unsigned char));
  if (V1BYTE == NULL) {
    printf("KS5 verifying memory allocation failed 1\n");
    ret = -10;
    goto cleanup;
  }

  V2BYTE = (unsigned char *)calloc(KS5_KAZ_DS_V2BYTES, sizeof(unsigned char));
  if (V2BYTE == NULL) {
    printf("KS5 verifying memory allocation failed 2\n");
    ret = -11;
    goto cleanup;
  }

  memcpy(V1BYTE, pk, KS5_KAZ_DS_V1BYTES);
  memcpy(V2BYTE, pk + KS5_KAZ_DS_V1BYTES, KS5_KAZ_DS_V2BYTES);

  mpz_import(V1, KS5_KAZ_DS_V1BYTES, 1, sizeof(unsigned char), 0, 0, V1BYTE);
  mpz_import(V2, KS5_KAZ_DS_V2BYTES, 1, sizeof(unsigned char), 0, 0, V2BYTE);

  // 3) Get signature=(S, m)
  if (smlen == KS5_KAZ_DS_SBYTES) {
  }
  mpz_import(S, KS5_KAZ_DS_SBYTES, 1, sizeof(unsigned char), 0, 0, sm);

...
int KS1_KAZ_DS_KeyGen(unsigned char *kaz_ds_verify_key,
  ...
  mpz_set_str(N, KS1_KAZ_DS_SP_N, 10);
  mpz_set_str(GRg, KS1_KAZ_DS_SP_GRg, 10);
  mpz_set_str(phiGRg, KS1_KAZ_DS_SP_PHIGRg, 10);
  mpz_set_str(phiphiGRg, KS1_KAZ_DS_SP_PHIPHIGRg, 10);
  mpz_set_str(phiGg, KS1_KAZ_DS_SP_PHIGg, 10);
  mpz_set_str(q, KS1_KAZ_DS_SP_q, 10);
  mpz_set_str(GRgq, KS1_KAZ_DS_SP_GRgq, 10);
  mpz_set_str(Q, KS1_KAZ_DS_SP_Q, 10);
  mpz_set_str(phiQ, KS1_KAZ_DS_SP_PHIQ, 10);
  mpz_set_str(qQ, KS1_KAZ_DS_SP_qQ, 10);

  int nphiGg = KS1_KAZ_DS_SP_nPHIGg;
...
#ifdef KS1_DEBUG
  printf("Alpha size : %lu, BSIZE : %lu, Total : %lu\n", ALPHASIZE, BSIZE,
         ALPHASIZE + BSIZE);
  printf("V1SIZE : %lu; V2SIZE : %lu, Total : %lu\n", V1SIZE, V2SIZE,
         V1SIZE + V2SIZE);
#endif
  ...
  memset(kaz_ds_sign_key, 0, KS1_CRYPTO_SECRETKEYBYTES * sizeof(unsigned char));

  // The following algo doesn't work on nfast
  // int je = KS1_CRYPTO_SECRETKEYBYTES - 1;
  // for (int i = BSIZE - 1; i >= 0; i--) {
  //  // kaz_ds_sign_key[je] = BBYTE[i];
  //  priv_control[je] = BBYTE[i];
  //  je--;
  //}

  // Fill up the buffer from left most and leave MSB zero
  // if output is less than KS1_KAZ_DS_BBYTES
  memcpy(&(kaz_ds_sign_key[KS1_CRYPTO_SECRETKEYBYTES - BSIZE]), BBYTE, BSIZE);

  // same algo as above
  memcpy(&(kaz_ds_sign_key[KS1_CRYPTO_SECRETKEYBYTES - KS1_KAZ_DS_BBYTES -
                           ALPHASIZE]),
         ALPHABYTE, ALPHASIZE);

  // je = KS1_CRYPTO_SECRETKEYBYTES - KS1_KAZ_DS_BBYTES - 1;
  // for (int i = ALPHASIZE - 1; i >= 0; i--) {
  //   // kaz_ds_sign_key[je] = ALPHABYTE[i];
  //   priv_control[je] = ALPHABYTE[i];
  //   je--;
  // }
  memset(kaz_ds_verify_key, 0,
         KS1_CRYPTO_PUBLICKEYBYTES * sizeof(unsigned char));

  memcpy(&(kaz_ds_verify_key[KS1_CRYPTO_PUBLICKEYBYTES - V2SIZE]), V2BYTE,
         V2SIZE);

  memcpy(&(kaz_ds_verify_key[KS1_CRYPTO_PUBLICKEYBYTES - KS1_KAZ_DS_V2BYTES -
                             V1SIZE]),
         V1BYTE, V1SIZE);

  // je = KS1_CRYPTO_PUBLICKEYBYTES - 1;
  // for (int i = V2SIZE - 1; i >= 0; i--) {
  //   kaz_ds_verify_key[je] = V2BYTE[i];
  //   je--;
  // }

  // je = KS1_CRYPTO_PUBLICKEYBYTES - KS1_KAZ_DS_V2BYTES - 1;
...
int KS1_KAZ_DS_SIGNATURE_DETACHED(unsigned char *signature,
  ...
  mpz_set_str(phiphiGRg, KS1_KAZ_DS_SP_PHIPHIGRg, 10);
  mpz_set_str(phiQ, KS1_KAZ_DS_SP_PHIQ, 10);
  mpz_set_str(GRgqQ, KS1_KAZ_DS_SP_GRgqQ, 10);
  mpz_set_str(phiGRgqQ, KS1_KAZ_DS_SP_PHIGRgqQ, 10);
  mpz_set_str(qQ, KS1_KAZ_DS_SP_qQ, 10);
  ...
  mpz_import(ALPHA, KS1_KAZ_DS_ALPHABYTES, 1, sizeof(unsigned char), 0, 0,
             ALPHABYTE);
  mpz_import(b, KS1_KAZ_DS_BBYTES, 1, sizeof(unsigned char), 0, 0, BBYTE);

  // 3) Compute HASHValue(m)
  unsigned char buf[KS1_CRYPTO_BYTES] = {0};
  HashMsg(m, mlen, buf);

  mpz_import(hashValue, KS1_CRYPTO_BYTES, 1, sizeof(unsigned char), 0, 0, buf);
  mpz_nextprime(hashValue, hashValue);

  // 4) Generate random r & ephemeral BETA
  KAZ_DS_RANDOM(KS1_KAZ_DS_SP_nPHIGg - 2, KS1_KAZ_DS_SP_nPHIGg - 1, r);
  mpz_nextprime(r, r);
  ...
  mpz_export(SBYTE, &SSIZE, 1, sizeof(unsigned char), 0, 0, S);

  memset(signature, 0, KS1_KAZ_DS_SBYTES * sizeof(unsigned char));
  memcpy(signature + (KS1_KAZ_DS_SBYTES - SSIZE), SBYTE, SSIZE);
...
int KS1_KAZ_DS_SIGNATURE(unsigned char *signature, unsigned long long *signlen,
  ...
  // 2) Get kaz_ds_sign_key=(ALPHA, b)
  unsigned char *ALPHABYTE =
      (unsigned char *)malloc((KS1_KAZ_DS_ALPHABYTES) * sizeof(unsigned char));
  unsigned char *BBYTE =
      (unsigned char *)malloc((KS1_KAZ_DS_BBYTES) * sizeof(unsigned char));

  for (int i = 0; i < KS1_KAZ_DS_ALPHABYTES; i++)
    ALPHABYTE[i] = 0;

  for (int i = 0; i < KS1_KAZ_DS_BBYTES; i++)
    BBYTE[i] = 0;
  ...
  for (int i = 0; i < KS1_KAZ_DS_BBYTES; i++) {
    BBYTE[i] = sk[i + KS1_KAZ_DS_ALPHABYTES];
  }

  mpz_import(ALPHA, KS1_KAZ_DS_ALPHABYTES, 1, sizeof(char), 0, 0, ALPHABYTE);
  mpz_import(b, KS1_KAZ_DS_BBYTES, 1, sizeof(char), 0, 0, BBYTE);

  // 3) Compute HASHValue(m)
  unsigned char buf[KS1_CRYPTO_BYTES] = {0};
  HashMsg(m, mlen, buf);

  mpz_import(hashValue, KS1_CRYPTO_BYTES, 1, sizeof(char), 0, 0, buf);
  mpz_nextprime(hashValue, hashValue);
...
int KS1_KAZ_DS_VERIFICATION_DETACHED(const unsigned char *m, unsigned int mlen,
  ...
  // 1) Get all system parameters
  mpz_set_str(N, KS1_KAZ_DS_SP_N, 10);
  mpz_set_str(g, KS1_KAZ_DS_SP_G, 10);
  mpz_set_str(Gg, KS1_KAZ_DS_SP_Gg, 10);
  mpz_set_str(R, KS1_KAZ_DS_SP_R, 10);
  mpz_set_str(GRg, KS1_KAZ_DS_SP_GRg, 10);
  mpz_set_str(q, KS1_KAZ_DS_SP_q, 10);
  mpz_set_str(Q, KS1_KAZ_DS_SP_Q, 10);
  mpz_set_str(phiQ, KS1_KAZ_DS_SP_PHIQ, 10);
  mpz_set_str(GRgQ, KS1_KAZ_DS_SP_GRgQ, 10);
  mpz_set_str(phiGRgQ, KS1_KAZ_DS_SP_PHIGRgQ, 10);
  mpz_set_str(GRgqQ, KS1_KAZ_DS_SP_GRgqQ, 10);
  mpz_set_str(qQ, KS1_KAZ_DS_SP_qQ, 10);
  mpz_set_str(phiqQ, KS1_KAZ_DS_SP_PHIqQ, 10);
  ...
  memcpy(V1BYTE, pk, KS1_KAZ_DS_V1BYTES);
  memcpy(V2BYTE, pk + KS1_KAZ_DS_V1BYTES, KS1_KAZ_DS_V2BYTES);

  mpz_import(V1, KS1_KAZ_DS_V1BYTES, 1, sizeof(unsigned char), 0, 0, V1BYTE);
...
int KS1_KAZ_DS_VERIFICATION(unsigned char *m, unsigned long long *mlen,
  ...
  // 2) Get kaz_ds_verify_key=(V1, V2)
  unsigned char *V1BYTE =
      (unsigned char *)malloc((KS1_KAZ_DS_V1BYTES) * sizeof(unsigned char));
  unsigned char *V2BYTE =
      (unsigned char *)malloc((KS1_KAZ_DS_V2BYTES) * sizeof(unsigned char));

  for (int i = 0; i < KS1_KAZ_DS_V1BYTES; i++)
    V1BYTE[i] = 0;
  for (int i = 0; i < KS1_KAZ_DS_V2BYTES; i++)
    V2BYTE[i] = 0;
  ...
  for (int i = 0; i < KS1_KAZ_DS_V2BYTES; i++) {
    V2BYTE[i] = pk[i + KS1_KAZ_DS_V1BYTES];
  }

  mpz_import(V1, KS1_KAZ_DS_V1BYTES, 1, sizeof(char), 0, 0, V1BYTE);
  mpz_import(V2, KS1_KAZ_DS_V2BYTES, 1, sizeof(char), 0, 0, V2BYTE);

  // 3) Get signature=(S, m)
  int len = smlen - KS1_KAZ_DS_SBYTES;

  unsigned char *SBYTE =
      (unsigned char *)malloc(KS1_KAZ_DS_SBYTES * sizeof(unsigned char));
  unsigned char *MBYTE = (unsigned char *)malloc(len * sizeof(unsigned char));

  for (int i = 0; i < KS1_KAZ_DS_SBYTES; i++)
    SBYTE[i] = 0;
  for (int i = 0; i < len; i++)
    MBYTE[i] = 0;
  ...
  for (int i = 0; i < len; i++) {
    MBYTE[i] = sm[i + KS1_KAZ_DS_SBYTES];
  }

  mpz_import(S, KS1_KAZ_DS_SBYTES, 1, sizeof(char), 0, 0, SBYTE);
